...
def register_routes(app):
    ...
    def report_print(vehicle_id, period):
        ...
        try:
            ...
            cur.execute(
                '''
                SELECT date, driver, purpose, odo_start, odo_end,
            ...
            for index, row in enumerate(trip_rows, start=1):
                ...
                if hasattr(d, 'strftime'):
                    ...
                    try:
                        ...
                        parsed = dt_date.fromisoformat(str(d))
                        date_str = parsed.strftime('%d.%m.%Y')
                    except Exception:
                        date_str = str(d)

                # Godziny
                t_start = (row['time_start'] or '').strip()
                t_end   = (row['time_end']   or '').strip()
                godziny = ''
                if t_start and t_end:
                    godziny = f'{t_start} – {t_end}'
                elif t_start:
                    godziny = f'{t_start} –'
                elif t_end:
                    godziny = f'– {t_end}'
                ...
                rows.append({
                    ...
                    'trip_km':   trip_km if trip_km is not None else '',
                    'godziny':   godziny,
